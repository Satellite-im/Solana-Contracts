...
(async function() {
    ...
    let stickerFactoryAccount = await createStickerFactory(connection, PAYER_ACCOUNT, stickerFactoryOwner);

    await waitForAccount(connection, stickerFactoryAccount.publicKey);

    let stickerFactoryData = await getStickerFactory(connection, stickerFactoryAccount.publicKey);

    console.log("New StickerFactory account was created and initialized: ", stickerFactoryAccount.publicKey.toBase58());

...
async function createDerivedAccount(connection, payerAccount, seedKey, seedString, index, addressTypeValue) {
  let base = await PublicKey.findProgramAddress([seedKey.toBytes()], SERVER_PROGRAM_ID);
  let addressToCreate = await PublicKey.createWithSeed(base[0], seedString+index, SERVER_PROGRAM_ID);
...
async function createServer(connection, payerAccount, dwellerAccount, name) {
  ...
  let dwellerServer = await createDerivedAccount(connection, payerAccount, dwellerAccount.publicKey, DWELLER_SERVER_SEED, dwellerData.servers, "dwellerServer");
  console.log("DwellerServer account created: ", dwellerServer.toBase58());
  let serverMembers = 0;
  let serverMember = await createDerivedAccount(connection, payerAccount, server.publicKey, SERVER_MEMBER_SEED, serverMembers, "serverMember");
  ...
  const transaction = new Transaction().add(
    ...
    initializeServer(dwellerAccount.publicKey, server.publicKey, dwellerServer, serverMember, name)
  );

...
async function initFriendRequest(requestFromToKey, requestToFromKey, friendInfoFromKey, friendInfoToKey, userFromKey) {
    return new TransactionInstruction({
        keys: [
...
async function createFriendRequest(connection, payerAccount, userFromAccount, userToKey, friendInfoFromKey, friendInfoToKey) {
    let friendInfoFromData = await getFriendInfo(connection, friendInfoFromKey);
    let params = {createAccount: {requestOutgoing: {index: friendInfoFromData.requests_outgoing}}};
    let requestFromAccount = await createDerivedAccount(connection, payerAccount, userFromAccount.publicKey, friendInfoFromData.requests_outgoing+OUTGOING_REQUEST, params);

    let friendInfoToData = await getFriendInfo(connection, friendInfoToKey);
    params = {createAccount: {requestIncoming: {index: friendInfoToData.requests_incoming}}};
    let requestToAccount = await createDerivedAccount(connection, payerAccount, userToKey, friendInfoToData.requests_incoming+INCOMING_REQUEST, params);

    let transaction = new Transaction().add(await initFriendRequest(requestFromAccount, requestToAccount, friendInfoFromKey, friendInfoToKey, userFromAccount.publicKey));

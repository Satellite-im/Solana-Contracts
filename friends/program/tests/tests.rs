...
async fn test_make_friend_request() {
    ...
    let request_from_info = state::Request::try_from_slice(&request_from_info_data.data.as_slice()).unwrap();
    ...
    let request_to_info = state::Request::try_from_slice(&request_to_info_data.data.as_slice()).unwrap();
